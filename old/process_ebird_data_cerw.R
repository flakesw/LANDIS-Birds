...
output_file <- "ebd_filtered_cerw.txt"
output_sampling <- "ebd_sampling_filtered.txt"
f_ebd <- "D:/Data/ebird_data/ebd_US_cerwar_relMar-2023/ebd_US_cerwar_relMar-2023.txt"
...
ebd_filters <-  auk_ebd(f_ebd, file_sampling = f_smp) %>%  
  ...
  auk_complete() %>%
  auk_date(date = c("*-06-15", "*-07-30")) %>% #TODO change for each species
...
ebird <- ebd_zf_df %>% 
  dplyr::select(checklist_id, observer_id, sampling_event_identifier,
         ...
         number_observers)
write_csv(ebird, "cerw_filtered.csv", na = "")

#-----------------------set up for GEE

ebird <- read.csv("cerw_filtered.csv")
ebird_sf <- read.csv("cerw_filtered.csv") %>% 
...
write.csv(ebird_ss, "cerw_subsampled.csv")
