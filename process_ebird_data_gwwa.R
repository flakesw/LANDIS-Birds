#analyze GWWA detection via GEE
...
output_file <- "ebd_filtered_gwwa.txt"
output_sampling <- "ebd_sampling_filtered.txt"
f_ebd <- "D:/Data/ebird_data/ebd_US_gowwar_201001_202301_relJul-2023/ebd_US_gowwar_201001_202301_relJul-2023.txt"
...
ebird <- ebd_zf_df %>% 
  dplyr::select(checklist_id, observer_id, sampling_event_identifier,
         ...
         number_observers)
write_csv(ebird, "gwwa_filtered.csv", na = "")

#-----------------------set up for GEE

ebird <- read.csv("gwwa_filtered.csv")
ebird_sf <- read.csv("gwwa_filtered.csv") %>% 
...
write.csv(ebird_ss, "gwwa_subsampled.csv")
