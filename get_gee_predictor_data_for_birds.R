...
#this is the newest version of script as of 11/13/2024

#------------------------------------------------------

#TODO:
# try restricting data to post-GEDI
...
# #which ebird data to use?
# ebird_ss_files <- list.files("./ebird", species, full.names = TRUE) %>%
#   `[`(grepl("csv", .)) %>%
#   `[`(grepl("subsampled_balanced", .))
# ebird_ss_file <- ebird_ss_files[2]
...
# i <- 1
# ebird_ss_group <- filter(ebird_ss, group == unique(ebird_ss$group)[i])
# ebird_buff_group <- filter(ebird_buff, group == unique(ebird_ss$group)[i])
# ebird_buff_albers_group <- filter(ebird_buff_albers, group == unique(ebird_ss$group)[i])
...
clim_buffer <- ifelse(buffer_scale > 2000, buffer_scale, 2000) #minimum climate buffer of 2km
...
combined <- left_join(dplyr::select(ebird_buff_albers %>% st_drop_geometry(), 
  ...
  st_drop_geometry()
combined %>%
  write_csv(., paste0("./environment_vars/", species, "_combined_data_bcr28_gee", buffer_scale, "_", Sys.Date(),".csv"))
