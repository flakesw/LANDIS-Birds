  ...
  for(buffer_scale in scale_range){
    ...
    for(group_select in unique(ebird_buff_albers$group)){
      ...
      combined <- climate_vars %>%
        ...
        left_join(dplyr::select(ebird_all_obs, species, checklist_id, 
                                time_observations_started, duration_minutes,
                                effort_radius_m, species_observed), by = "checklist_id") %>%
        ungroup() %>%
        st_as_sf() %>%
      ...
      combined %>%
        write_csv(., paste0("./environment_vars/gee_temp/", species, "_combined_data_bcr28_gee_", buffer_scale, "group", group_select, ".csv"))
    }
    
    scale_elapsed <- Sys.time() - scale_start_time
      
    print(paste0("Scale ", buffer_scale, " done! It took ", round(scale_elapsed, 2), " ", attr(scale_elapsed, "units")))
