...
output_file <- "ebd_filtered_woth.txt"
output_sampling <- "ebd_sampling_filtered.txt"
f_ebd <- "./ebird_data/ebd_US_woothr_relMar-2023/ebd_US_woothr_relMar-2023.txt"
f_smp <- "./ebird_data/ebd_sampling_relMar-2023/ebd_sampling_relMar-2023.txt"
...
ebird <- ebd_zf_filtered %>% 
  dplyr::select(checklist_id, observer_id, sampling_event_identifier,
         ...
         number_observers)
write_csv(ebird, "woth_filtered.csv", na = "")


#-----------------------set up for GEE

ebird <- read.csv("woth_filtered.csv")
ebird_sf <- read.csv("woth_filtered.csv") %>% 
...
write.csv(ebird_ss, "woth_subsampled.csv")
