...
ebird_buff_treemap <- ebird_buff %>%
  sf::st_transform(crs(tm2))
ebird_treemap_extract <- terra::extract(treemap, vect(ebird_buff_treemap), raw = TRUE) %>%
  as.data.frame()
...
tree_hardwood <- fia_trees %>%
  ...
  mutate(H = ifelse(site_bio == 0 | is.na(site_bio), NA, H)) #if there's no biomass, make proportion oak/hickory = NA
